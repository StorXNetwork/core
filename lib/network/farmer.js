  ...
  async.series([
    ...
    (next) => {
      ...
      let body = {
        ...
        paymentAddress: this._options.paymentAddress
      }
...
FarmerInterface.prototype.updateBridgeContactForNodeStatus = function(callback) {
  const headers = {};
  const body = {
    spaceAvailable: this.spaceAvailable
  };
  ...
  async.eachSeries(this.bridges.values(), (bridge, next) => {
    ...
    if (err) {
      return this._logger.error('Unable to update bridge for node status update', err);
    }
  });
}
